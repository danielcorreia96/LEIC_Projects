...
public class SupplierPortImpl implements SupplierPortType {
	...
	public ProductView getProduct(String productId) throws BadProductId_Exception {
		...
		// check if productId is composed only of numbers and+or letters
	...
	public List<ProductView> searchProducts(String descText) throws BadText_Exception {
		if (descText == null || descText.trim().length() == 0)
			throwBadText("Bad description!");

		List<ProductView> productList = listProducts();
		List<ProductView> resultList = new ArrayList<ProductView>(); 
		for (ProductView pv : productList) {
			String desc = pv.getDesc();
			if (desc.contains(descText)) {
				resultList.add(pv);
			}
		}
	...
	public String buyProduct(String productId, int quantity)
		...
		if (!productId.matches("[A-Za-z0-9]+"))
			throwBadProductId("Product identifier cannot be empty or whitespace!");
		if (quantity <= 0) {
			throwBadQuantity("Product quantity cannot be negative or zero!"); 	
		}
		Supplier s = Supplier.getInstance();
		
		try {
			if (s.productExists(productId))
				return s.buyProduct(productId, quantity);
		} catch (QuantityException e) {
			throwInsufficientQuantity("Tried to buy "+quantity+" units of product but failed.");
		}
		return null;
